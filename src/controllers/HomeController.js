...
ctrl.profile = async (req, res) => {
    ...
    if (req.user.idTipoEntidad == 2) {
        ...
        return res.render('postlog/inmoprofile', {totalInmuebles: inmueble.length ,repre: rep, inmuebles: inmueble, totalReuniones: reuniones.length, totalReservas: reservas.length });
    }
...
ctrl.misreservas = async (req, res) => {
    let noReservas;
    const reunion = await db.getUserReunion(req.user.idEntidad)
    reunion.forEach(element => {
        element.fecha = element.fecha.toLocaleDateString()
    ...
    res.render('postlog/inmoreservas', {reunion, noReservas});
}
